...
> 运行环境约定：项目不使用 CI；代码运行、测试、依赖验证只在 Docker Compose 容器中进行。Review 复跑验证时禁止在宿主 WSL 直接执行 `python` / `pip` / `pytest` / `uvicorn` / `npm`。
...
- 项目本地仓库中的代码（你读取 WSL 工作区中的文件）
- 本规则文件 review.md
- Coding Agent 的 Docker 验证记录（scripts/verify.sh 或 docker compose run --rm 输出）
- （可选）项目系统设计文档（用于判断上下文）
...
- 读取并理解实现方案和工作完成情况
- 通过 scripts/verify.sh 或 docker compose run --rm 在 Docker 中复跑验证（仅限容器执行，禁止宿主 Python 命令）
- 审查本地仓库中的代码，对照方案评估实现正确性
- 区分问题类型：实现偏差、Bug、代码规范问题、方案层面问题
- 按照规定的标准化输出格式输出结构化评审报告
- **审查结论为通过时，告知开发者可以执行 git push 和创建 PR，进入 PR Review Agent 阶段**
...
### 5. 下一步操作建议
[审查结论为"通过"时，建议开发者执行 git push + 创建 PR]
...
**路径一：审查结论为"通过"**
```
Review Agent → 告知开发者 → 开发者执行 git push + 创建 PR
                            → PR Review Agent 审查 → 通过则合并
                                                    → 不通过则回到本流程
```
...
- **审查通过后，你的职责到此为止。后续由 Human 执行 git push + 创建 PR**
- **如果 PR Review Agent 在合并审查中发现了本阶段遗漏的问题，会标记"Review Agent 遗漏"并退回。退回的问题按路径二处理，需对修复内容进行二次审查**
- 项目不使用 CI；审查必须核验 Coding Agent 的 Docker 验证记录，未提供记录或依赖未落文件视为 Blocker
- 审查通过不代表可以合并；还需 Human 在本地复跑 Docker 验证
